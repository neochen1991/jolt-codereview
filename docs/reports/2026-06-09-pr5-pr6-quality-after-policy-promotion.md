...
- Overall False Positive Rate: 0.0000 (0/49)
- Meets 90/10 Target: yes

| PR | Run | Findings | Matched | Missing | Recall | FP | FP Rate | Tool Obs | Target |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| PR5 dynamic risk policy | `run_dfc955b2efda4cac` | 20 | 19 | 1 | 0.9500 | 0 | 0.0000 | 11 | yes |
| PR6 archive import/export | `run_ad3f2a565af94a8c` | 29 | 20 | 0 | 1.0000 | 0 | 0.0000 | 14 | yes |

## PR5 dynamic risk policy

- Status: waiting_confirmation
- Candidate Quality: `{"expert_candidate_count": 24, "verifier_accepted_count": 17, "verifier_rejected_count": 7, "verifier_rejected_reason_counts": {"evidence_not_in_source": 5, "source_contradicts_return_null": 2}, "tool_observation_count": 11, "tool_observation_rejected_not_on_diff_count": 0, "promoted_tool_candidate_count": 8, "judge_rejected_count": 7, "final_finding_count": 20, "candidate_rejected_count": 14}`
- Tool Observation States: `{"adopted_final": 10, "candidate": 1}`
...
| --- | --- | --- | --- |
| PR5-01 | Hardcoded risk admin key | matched | 源码中硬编码敏感密钥 (security_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:22) |
| PR5-02 | Admin key passed in query parameter | matched | 敏感凭据通过 URL 查询参数传输 (security_agent, src/main/java/com/joltbenchmark/payment/api/DynamicRiskPolicyController.java:27) |
...
| PR5-09 | lastPolicy global state pollutes tenants | matched | 共享非线程安全对象 (coding_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:23) |
| PR5-10 | paymentId miss falls back to findAll first order | matched | 跨租户数据越权访问 (security_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:81) |
| PR5-11 | evaluate does not verify merchant ownership | matched | 风险策略写接口缺少入参校验 (backend_agent, src/main/java/com/joltbenchmark/payment/api/DynamicRiskPolicyController.java:23) |
| PR5-12 | previewForMerchant leaks global payment summary | matched | 跨租户数据越权访问 (security_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:81) |
...
| PR5-16 | BigDecimal equals scale mismatch | matched | BigDecimal 不应使用 double/float 构造 (coding_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:67) |
| PR5-17 | Default SpEL expression expands executable surface | missing | - |
| PR5-18 | lastPolicyAgeSeconds can NPE | matched | BigDecimal 不应使用 double/float 构造 (coding_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:67) |
...
| PR5-20 | Risk policy APIs lack real authz/audit approval | matched | 敏感凭据通过 URL 查询参数传输 (security_agent, src/main/java/com/joltbenchmark/payment/api/DynamicRiskPolicyController.java:27) |
