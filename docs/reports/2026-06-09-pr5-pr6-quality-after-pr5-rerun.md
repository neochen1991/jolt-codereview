...
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| PR5 dynamic risk policy | `run_4cbe43d7d70a4f25` | 26 | 19 | 1 | 0.9500 | 0 | 0.0000 | 11 | yes |
| PR6 archive import/export | `run_ad3f2a565af94a8c` | 29 | 20 | 0 | 1.0000 | 0 | 0.0000 | 14 | yes |

## PR5 dynamic risk policy

- Status: waiting_confirmation
- Candidate Quality: `{"expert_candidate_count": 32, "verifier_accepted_count": 26, "verifier_rejected_count": 6, "verifier_rejected_reason_counts": {"source_contradicts_return_null": 5, "source_contradicts_bigdecimal_double_constructor": 1}, "tool_observation_count": 11, "tool_observation_rejected_not_on_diff_count": 0, "promoted_tool_candidate_count": 9, "judge_rejected_count": 13, "final_finding_count": 26, "candidate_rejected_count": 19}`
- Tool Observation States: `{"adopted_final": 9, "candidate": 2}`
...
| --- | --- | --- | --- |
| PR5-01 | Hardcoded risk admin key | matched | 源码中写入管理密钥并使用固定种子随机数 (security_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:22) |
| PR5-02 | Admin key passed in query parameter | matched | 配置或代码中包含明文密钥 (security_agent, src/main/java/com/joltbenchmark/payment/api/DynamicRiskPolicyController.java:27) |
...
| PR5-07 | Static HashMap policy cache is not thread safe | matched | Spring 单例服务中使用 static HashMap 保存可变状态 (coding_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:23) |
...
| PR5-09 | lastPolicy global state pollutes tenants | matched | Spring 单例服务中使用 static HashMap 保存可变状态 (coding_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:23) |
| PR5-10 | paymentId miss falls back to findAll first order | matched | merchant 预览接口没有按 merchantId 过滤数据 (coding_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:81) |
| PR5-11 | evaluate does not verify merchant ownership | matched | 新增写接口缺少入参校验 (backend_agent, src/main/java/com/joltbenchmark/payment/api/DynamicRiskPolicyController.java:23) |
| PR5-12 | previewForMerchant leaks global payment summary | matched | 商户预览接口忽略 merchantId 查询全量支付单 (backend_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:80) |
| PR5-13 | preview returns active policy from another merchant | matched | merchant 预览接口没有按 merchantId 过滤数据 (coding_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:81) |
| PR5-14 | java.util.Random used for risk sampling | matched | 安全或风控决策使用弱随机源 (security_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:25) |
| PR5-15 | SecureRandom initialized with fixed seed | matched | 安全或风控决策使用弱随机源 (security_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:25) |
| PR5-16 | BigDecimal equals scale mismatch | matched | BigDecimal.equals 会因 scale 不同导致金额判断错误 (coding_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:67) |
| PR5-17 | Default SpEL expression expands executable surface | matched | 不可信风控表达式通过 SpEL 动态执行 (security_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:58) |
| PR5-18 | lastPolicyAgeSeconds can NPE | matched | BigDecimal.equals 会因 scale 不同导致金额判断错误 (coding_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:67) |
| PR5-19 | Policy priority is stored but ignored | missing | - |
| PR5-20 | Risk policy APIs lack real authz/audit approval | matched | 预览接口读取并返回跨商户全量支付摘要 (security_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:81) |

Unmapped final findings:
- Preview/read-only endpoint loads all records with findAll and returns global sum (security_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:80)
