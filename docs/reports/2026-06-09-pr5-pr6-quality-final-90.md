...
- Overall Recall: 0.9500 (38/40)
- Overall False Positive Rate: 0.0370 (2/54)
- Meets 90/10 Target: yes

| PR | Run | Findings | Matched | Missing | Recall | FP | FP Rate | Tool Obs | Target |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| PR5 dynamic risk policy | `run_3213ad336b5e4ef0` | 25 | 18 | 2 | 0.9000 | 2 | 0.0800 | 9 | yes |
| PR6 archive import/export | `run_ad3f2a565af94a8c` | 29 | 20 | 0 | 1.0000 | 0 | 0.0000 | 14 | yes |

## PR5 dynamic risk policy

- Status: waiting_confirmation
- Candidate Quality: `{"expert_candidate_count": 25, "verifier_accepted_count": 25, "verifier_rejected_count": 0, "verifier_rejected_reason_counts": {}, "tool_observation_count": 9, "tool_observation_rejected_not_on_diff_count": 0, "promoted_tool_candidate_count": 6, "judge_rejected_count": 7, "final_finding_count": 25, "candidate_rejected_count": 7}`
- Tool Observation States: `{"adopted_final": 7, "candidate": 2}`
...
| PR5-02 | Admin key passed in query parameter | matched | 敏感管理凭据通过 RequestParam 传递 (security_agent, src/main/java/com/joltbenchmark/payment/api/DynamicRiskPolicyController.java:27) |
...
| PR5-07 | Static HashMap policy cache is not thread safe | matched | 静态可变状态在并发请求下会产生不一致结果 (coding_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:23) |
| PR5-08 | Fixed active cache key causes merchant overwrite | matched | 静态可变状态在并发请求下会产生不一致结果 (coding_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:23) |
| PR5-09 | lastPolicy global state pollutes tenants | matched | 静态可变状态在并发请求下会产生不一致结果 (coding_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:23) |
| PR5-10 | paymentId miss falls back to findAll first order | matched | evaluate 方法在订单不存在时返回默认允许结果，违反失败关闭原则 (backend_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:47) |
| PR5-11 | evaluate does not verify merchant ownership | matched | 敏感管理凭据通过 RequestParam 传递 (security_agent, src/main/java/com/joltbenchmark/payment/api/DynamicRiskPolicyController.java:27) |
| PR5-12 | previewForMerchant leaks global payment summary | matched | evaluate 方法在订单不存在时返回默认允许结果，违反失败关闭原则 (backend_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:47) |
| PR5-13 | preview returns active policy from another merchant | matched | 静态可变状态在并发请求下会产生不一致结果 (coding_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:23) |
...
| PR5-16 | BigDecimal equals scale mismatch | matched | BigDecimal 金额使用 equals 比较会因 scale 不同导致误判 (low_level_defect_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:67) |
| PR5-17 | Default SpEL expression expands executable surface | missing | - |
| PR5-18 | lastPolicyAgeSeconds can NPE | matched | BigDecimal 金额使用 equals 比较会因 scale 不同导致误判 (low_level_defect_agent, src/main/java/com/joltbenchmark/payment/service/DynamicRiskPolicyService.java:67) |
| PR5-19 | Policy priority is stored but ignored | matched | 风控策略的核心业务概念长期使用裸类型表达 (ddd_agent, src/main/java/com/joltbenchmark/payment/domain/DynamicRiskPolicy.java:6) |
| PR5-20 | Risk policy APIs lack real authz/audit approval | matched | 敏感管理凭据通过 RequestParam 传递 (security_agent, src/main/java/com/joltbenchmark/payment/api/DynamicRiskPolicyController.java:27) |

Unmapped final findings:
- RiskPlugin 直接暴露 PaymentOrder 聚合导致风控扩展与支付上下文耦合 (ddd_agent, src/main/java/com/joltbenchmark/payment/service/RiskPlugin.java:6)
