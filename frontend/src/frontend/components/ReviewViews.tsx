...
export function ReviewQualityCard({ quality }: { quality?: Record<string, unknown> }) {
  ...
  const v2Quality = quality?.v2_quality_status && typeof quality.v2_quality_status === "object" ? quality.v2_quality_status as Record<string, unknown> : {};
  ...
  const v2Status = String(v2Quality.status || "v2_unlabeled");
  const v2StatusLabels: Record<string, string> = {
    v2_unlabeled: "v2 未标注",
    v2_evaluating: "v2 评估中",
    v2_validated: "v2 已验证"
  };
  const missingEvidence = Array.isArray(v2Quality.missing_evidence) ? v2Quality.missing_evidence.map(String) : [];
  return (
    <div className="coverage-card review-quality-card" data-context-status={contextStatus}>
      ...
      <p><Code2 size={15} /><span>Context Health</span><em>v2 · 源码 {formatCoveragePercent(context.source_fetch_rate)} · Diff {formatCoveragePercent(context.diff_assignment_rate)}</em><strong>{contextStatus} / {contextLabels[contextStatus]}</strong></p>
      <p className={v2Status !== "v2_validated" ? "quality-warning" : ""}><ShieldCheck size={15} /><span>v2 Quality</span><em>{String(v2Quality.labeled_count || 0)} 条标注 · 精确率 {formatCoveragePercent(v2Quality.precision_value)} · {String(v2Quality.precision_confidence || "unlabeled")}</em><strong>{v2StatusLabels[v2Status] || v2Status}</strong></p>
      <p className={missingEvidence.length ? "quality-warning" : ""}><AlertTriangle size={15} /><span>验证缺口</span><em>{missingEvidence.length ? missingEvidence.join("；") : "用户标注、Gold 评估与质量门禁均完整"}</em><strong>{missingEvidence.length}</strong></p>
