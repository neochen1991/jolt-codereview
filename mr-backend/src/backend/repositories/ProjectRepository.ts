...
export class ProjectRepository {
  ...
  ensureReviewDefaults(targetProjectId: string, sourceProjectId = "project_default") {
    const existing = this.db.prepare("SELECT 1 FROM review_policy WHERE project_id = ? LIMIT 1").get(targetProjectId);
    if (existing) return;
  ...
  private cloneReviewDefaults(sourceProjectId: string, targetProjectId: string) {
    ...
    this.db.prepare(`
      ...
      SELECT 'rules_' || substr(md5(random()::text || clock_timestamp()::text), 1, 16), ?, name, version, scope_json, content, status
      FROM rule_sets
      WHERE project_id = ?
      ON CONFLICT DO NOTHING
    ...
    this.db.prepare(`
      ...
      SELECT 'rule_doc_' || substr(md5(random()::text || clock_timestamp()::text), 1, 16), ?, name, doc_type, content, version, status
      FROM rule_documents
      WHERE project_id = ?
      ON CONFLICT DO NOTHING
    `).run(targetProjectId, sourceProjectId);
    this.db.prepare(`
      ...
      SELECT 'skill_' || substr(md5(random()::text || clock_timestamp()::text), 1, 16), ?, skill_key, name, description, content, version, status
      FROM custom_skills
      WHERE project_id = ?
      ON CONFLICT DO NOTHING
    `).run(targetProjectId, sourceProjectId);
    this.db.prepare(`
      ...
      SELECT 'skill_asset_' || substr(md5(random()::text || clock_timestamp()::text), 1, 16), ?, skill_key, asset_path, asset_type, content, executable
    ...
    this.db.prepare(`
      ...
      SELECT 'profile_' || substr(md5(random()::text || clock_timestamp()::text), 1, 16), ?, agent_key, display_name, role_profile, responsibility_scope, excluded_scope,
        enabled, min_confidence, max_findings, max_llm_calls, max_tool_calls, output_schema_version
    ...
    this.db.prepare(`
      ...
      SELECT 'agent_' || substr(md5(random()::text || clock_timestamp()::text), 1, 16), ?, agent_id, display_name, enabled, applies_to_json, tools_json,
        skills_json, rule_sets_json, requires_deepagents, min_confidence, max_findings_per_mr
      FROM agent_configs
      WHERE project_id = ?
      ON CONFLICT DO NOTHING
    ...
    this.db.prepare(`
      ...
      SELECT 'skill_binding_' || substr(md5(random()::text || clock_timestamp()::text), 1, 16), ?, agent_key, skill_key, priority, enabled
    ...
    this.db.prepare(`
      ...
      SELECT 'tool_binding_' || substr(md5(random()::text || clock_timestamp()::text), 1, 16), ?, agent_key, tool_name, permission_level, max_calls, enabled
    ...
    for (const sourceDoc of sourceDocs) {
      ...
      this.db.prepare(`
        ...
        SELECT 'rule_binding_' || substr(md5(random()::text || clock_timestamp()::text), 1, 16), ?, agent_key, ?, priority
