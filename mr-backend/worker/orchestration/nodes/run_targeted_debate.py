                ...
                try:
                    def invoke_debate(seed: int) -> dict[str, Any]:
                        ...
                        return invoke_with_parameter_fallback(
                            payload,
                            lambda active: invoke_openai_chat(base_url=base_url, api_key=str(api_key), payload=active, timeout_seconds=timeout_seconds, stream=stream_enabled, transport=http_json, retry=call_with_retry),
                            on_downgrade=lambda parameter: recorder.event(span_id, "llm_parameter_downgraded", f"{model} 辩论调用移除不兼容参数：{parameter}", {"provider": provider, "model": model, "parameter": parameter, "operation": "targeted_debate"}),
                        )
