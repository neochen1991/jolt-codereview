...
const DEFAULT_MR_ID = process.env.MR_ID || "mr_repo_github_performance_rule_doc_eval_9501";
const MIN_STRICT_RECALL = Number(process.env.MIN_RECALL || "0.8");
const MAX_STRICT_FP_RATE = Number(process.env.MAX_FP_RATE || "0.25");
...
function renderMarkdown(report) {
  const lines = [
    ...
    `- Performance Agent Findings: ${report.performance_agent_finding_count}`,
    `- Custom Rule Coverage By Performance Agent: ${formatPercent(report.performance_agent_custom_rule_recall)}`,
...
const binding = db.prepare(`
  ...
  WHERE rd.id = ?
    AND erb.project_id = ?
    AND erb.agent_key = 'performance_agent'
`).get(ruleDocumentId, projectId);
if (!binding) throw new Error(`performance_agent is not bound to rule document: ${ruleDocumentId}`);
...
const performanceAgentFindings = findings.filter((finding) => finding.agent_id === "performance_agent");
const performanceAgentCustomRules = new Set(
  performanceAgentFindings.flatMap((finding) => finding.covered_rules).filter((rule) => expectedRules.has(rule))
);
...
const performanceAgentCustomRuleRecall = expectedRules.size ? performanceAgentCustomRules.size / expectedRules.size : 1;
...
const report = {
  ...
  bound_document: {
    ...
    rule_heading_count: (String(binding.content || "").match(/^##\s+PERFDOC-/gm) || []).length,
  },
  ...
  performance_agent_finding_count: performanceAgentFindings.length,
  performance_agent_custom_rule_count: performanceAgentCustomRules.size,
  performance_agent_custom_rule_recall: Number(performanceAgentCustomRuleRecall.toFixed(4)),
  meets_target: strictRecall >= MIN_STRICT_RECALL && strictFpRate <= MAX_STRICT_FP_RATE && performanceAgentCustomRuleRecall >= MIN_STRICT_RECALL,
...
const reportPath = path.join(reportDir, "2026-06-16-performance-rule-doc-binding-quality-report.md");
...
if (strictRecall < MIN_STRICT_RECALL) throw new Error(`performance rule doc strict recall below target: ${report.strict_recall}`);
if (strictFpRate > MAX_STRICT_FP_RATE) throw new Error(`performance rule doc strict false positive rate above target: ${report.strict_fp_rate}`);
if (performanceAgentCustomRuleRecall < MIN_STRICT_RECALL) {
  throw new Error(`performance agent custom rule recall below target: ${report.performance_agent_custom_rule_recall}`);
}
