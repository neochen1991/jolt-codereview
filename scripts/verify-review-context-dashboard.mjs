...
for (const label of ["Context Health", "v2 Quality", "验证缺口", "Quality Funnel", "Skill Checkpoint", "Reproducibility", "未闭环 Candidate"]) {
  if (!uiSource.includes(label)) throw new Error(`review quality UI missing ${label}`);
}
for (const status of ["v2_unlabeled", "v2_evaluating", "v2_validated"]) {
  if (!routeSource.includes(status) || !uiSource.includes(status)) throw new Error(`v2 quality dashboard missing status ${status}`);
}
for (const field of ["labeled_count", "precision_value", "precision_confidence", "missing_evidence"]) {
  if (!routeSource.includes(field)) throw new Error(`v2 quality API missing ${field}`);
}
