...
export function seed(db: Db) {
  db.prepare(`
    ...
    VALUES ('user_local_admin', 'local-admin', '本机管理员', 'local@example.com', 'active')
  `).run();
  ...
  const expertProfiles = [
    ...
    },
    {
      ...
      role_profile: "DDD 设计专家，聚焦领域概念、聚合边界、业务不变量和上下文边界。",
      responsibility_scope: "只检视领域模型、应用服务、领域服务、仓储、事件和业务规则表达问题。",
  ...
  const agents = [
    ...
    },
    {
      ...
      applies_to: {
        persona: "DDD 设计专家，关注领域概念、聚合边界和业务不变量。",
        review_scope: "只检视领域建模、聚合、值对象、领域服务、应用服务、仓储和上下文边界。",
        exclusive_scope: "ddd_design",
        languages: ["python", "typescript", "javascript"],
        paths: ["backend/**", "services/**", "domain/**", "src/**"],
        triggers: ["domain", "aggregate", "entity", "value_object", "repository", "service", "event"]
      },
      tools: [],
